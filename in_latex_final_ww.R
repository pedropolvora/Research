...
## 2011-2016
date_price <- read.csv("data_ww.csv", sep=",")
price <- ts(date_price[121:1685,2], start = c(2011), frequency = 261)
log_r <- ts(diff(log(date_price[121:1685,2])), start = c(2011), frequency = 261)
...
## without ts()
a <- diff(log(date_price[121:1685,2]))
par(mfcol=c(2,1))
...
lrfinal.order ## ARIMA(3,0,4)
plot(acf(resid(lrfinal.arima)), ylim = c(-0.1,0.5), main = "ACF of Residuals of an ARIMA(3,0,4)")
plot(acf(resid(lrfinal.arima)^2), ylim = c(-0.1,0.5), main = "ACF of Squared Residuals of an ARIMA(3,0,4)") ## arch effect
